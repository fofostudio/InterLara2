        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Estadísticas de Usuarios</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3">
                            <div class="card bg-primary text-white">
                                <div class="card-body">
                                    ...
                                    <p class="card-text h2">{{ $availableUsers }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card bg-success text-white">
                                <div class="card-body">
                                    ...
                                    <p class="card-text h2">{{ $totalMaxUsers }}</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-12">
                            <button type="button" class="btn btn-primary btn-round" data-toggle="modal"
                                data-target="#userModal" onclick="resetUserForm()">
            ...
            <form id="userForm" action="{{ route('user.store') }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <small class="form-text text-muted">Dejar en blanco para mantener la contraseña actual al
                            editar.</small>
                    </div>
                    <div class="form-group">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#userForm').on('submit', function(e) {
                ...
                var email = (firstName.charAt(0) + lastName).toLowerCase() + '@appinter.co';
                $('#email').val(email);

                // Habilitar temporalmente los campos de contraseña si están deshabilitados
                var passwordField = $('#password');
                var passwordConfirmationField = $('#password_confirmation');
                var wereDisabled = passwordField.prop('disabled');

                if (wereDisabled) {
                    passwordField.prop('disabled', false);
                    passwordConfirmationField.prop('disabled', false);
                }

                var formData = form.serialize();

                // Volver a deshabilitar los campos si estaban deshabilitados
                if (wereDisabled) {
                    passwordField.prop('disabled', true);
                    passwordConfirmationField.prop('disabled', true);
                }
        ...
        function handleRoleChange() {
            ...
            if (roleId == operatorRoleId) {
                var defaultPassword = 'SecretInter'; // Puedes cambiar esto por la contraseña por defecto que desees
                $('#password').val(defaultPassword);
                $('#password_confirmation').val(defaultPassword);
